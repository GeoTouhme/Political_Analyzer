...
class NewsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        encoding = self.tokenizer(
            title, add_special_tokens=True, max_length=self.max_len,
...
def train_epoch(model, data_loader, optimizer, device, scheduler, n_examples):
    ...
    for batch_idx, d in enumerate(data_loader):
        ...
        # Verbose output every 5 batches (~1 minute on CPU)
        if batch_idx % 5 == 0:
...
def main():
    print("--- TRANSFORMER TRAINING START (V4 - VERBOSE) ---")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch + 1} DONE - Loss: {train_loss:.4f} Acc: {train_acc:.4f}")
        sys.stdout.flush()
